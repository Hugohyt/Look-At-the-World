...
- (void)setIconAndName {
    ...
    self.avatarImageBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [self addSubview:self.nameLabel];
    [self addSubview:self.avatarImageBtn];
    
    [self.avatarImageBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.offset(100);
        make.width.and.height.equalTo(self.mas_width).multipliedBy(0.4);
        make.left.equalTo(self.mas_right).multipliedBy(0.3);
    }];
    self.avatarImageBtn.layer.cornerRadius = self.frame.size.width * 0.2;
    self.avatarImageBtn.layer.masksToBounds = YES;
    
    [self.nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.avatarImageBtn.mas_bottom).offset(20);
        make.width.equalTo(self.avatarImageBtn).multipliedBy(1.5);
