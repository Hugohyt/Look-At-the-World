...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.previousViewControllerView];
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture {
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        // 根据下拉距离调整当前视图的透明度
//        CGFloat offset = frame.origin.y - self.initialFrame.origin.y;
//        CGFloat maxOffset = self.audioPlayView.frame.size.height;
//        CGFloat alpha = offset / maxOffset;
        
        // 根据下拉距离调整上个视图控制器视图的透明度
        //        self.previousViewControllerView.alpha = alpha;
        ...
        if (distance > self.audioPlayView.frame.size.height / 3) {
            [UIView animateWithDuration:0.3 animations:^{
                ...
                self.previousViewControllerView.alpha = 1;
                self.audioPlayView.frame = CGRectMake(0, self.audioPlayView.frame.size.height, self.audioPlayView.frame.size.width, self.audioPlayView.frame.size.height);
            } completion:^(BOOL finished) {
                [self dismissViewControllerAnimated:NO completion:nil];
            }];
        } else {
            [UIView animateWithDuration:0.3 animations:^{
                self.audioPlayView.alpha = 1;
                //                self.previousViewControllerView.alpha = 0;
