...
//        }];
//    });
    self.authorization = [NSString stringWithFormat:@"Bearer %@", [[ManagerPost sharedManager] getModel].token ];
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    if (!self.isTexting) {
        [UIView animateWithDuration:[userInfo[UIKeyboardAnimationDurationUserInfoKey] doubleValue]
                         ...
                         animations:^{
            [self.commentTableView setContentOffset:CGPointMake(0, self.commentTableView.contentOffset.y + offset)];
...
- (void)keyboardWillHide:(NSNotification *)notification {
    ...
    if (self.isTexting) {
        ...
        [UIView animateWithDuration:[userInfo[UIKeyboardAnimationDurationUserInfoKey] doubleValue]
                            ...
                            options:([userInfo[UIKeyboardAnimationCurveUserInfoKey] integerValue] << 16)
                         animations:^{
            [self.commentTableView setContentOffset:CGPointMake(0, self.commentTableView.contentOffset.y - offset) animated:YES];
            [self.view layoutIfNeeded];
        } completion:^(BOOL finished) {
            self.isTexting = NO;
        }];
...
- (void)sendReplyWithFlag:(NSInteger)flag {
    ...
    [manager POST:@"https://travel.knoci.cn/comment/commit" parameters:parameters headers:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [self.commentTableView reloadSections:[NSIndexSet indexSetWithIndex:self.replyedSection + 2] withRowAnimation:UITableViewRowAnimationAutomatic];
    ...
    [manager POST:@"https://travel.knoci.cn/comment/commit" parameters:parameters headers:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSLog(@"%@",self.commentArray);
                    ...
                    NSIndexSet *newSectionIndex = [NSIndexSet indexSetWithIndex:0];
                    [self.commentTableView insertSections:newSectionIndex
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        [self replyComment:self.commentArray[indexPath.section] AtSection:indexPath.section];
    } else if (indexPath.row != 0) {
        [self replyReply:self.commentArray[indexPath.section].replies[indexPath.row - 1] AtSection:indexPath.section];
    } else {
        [self.inputTextView resignFirstResponder];
    }
}
...
- (void)setTableView {
    self.commentTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, 394, 900) style:UITableViewStyleGrouped];
    self.commentTableView.delegate = self;
    self.commentTableView.dataSource = self;
    self.commentTableView.rowHeight = UITableViewAutomaticDimension;
    self.commentTableView.estimatedRowHeight = 800;
    self.commentTableView.backgroundColor = [UIColor whiteColor];
    self.commentTableView.sectionHeaderHeight = 2;
    self.commentTableView.sectionFooterHeight = 2;
    
    [self.commentTableView registerClass:[CommentCell class] forCellReuseIdentifier:@"CommentCell"];
    [self.commentTableView registerClass:[ReplyTableViewCell class] forCellReuseIdentifier:@"ReplyTableViewCell"];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
        if (self.commentArray[section].replies.count == 0) {
            return 1;
        } else {
            if (self.commentArray[section].isExpand == NO) {
                return MIN(self.commentArray[section].replies.count + 1, 3);
            } else {
                return self.commentArray[section].replies.count + 1;
            }
        }
}

- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    if (self.commentArray[section].replies.count > 2) {
        return 40;
    } else {
        return 0;
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    if (self.commentArray[section].replies.count > 2) {
        ...
        if (self.commentArray[section].isExpand == NO) {
            [expandLabel setText:[NSString stringWithFormat:@"—— 点击展开%ld条评论", self.commentArray[section].replies.count]];
        } else {
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
        if (indexPath.row == 0) {
            ...
            [commentCell.nameLabel setText:self.commentArray[indexPath.section].name];
            [commentCell.timeLabel setText:self.commentArray[indexPath.section].time];
            [commentCell.commentTextView setText:self.commentArray[indexPath.section].content];
            [commentCell.nameIcon sd_setImageWithURL:[NSURL URLWithString:self.commentArray[indexPath.section].avatar]];
            ...
            replyTableViewCell.selectionStyle = UITableViewCellSelectionStyleNone;
            Comment* reply = self.commentArray[indexPath.section].replies[indexPath.row - 1];
            if ([reply.replyname isEqual:self.commentArray[indexPath.section].name]) {
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)gestureRecognizer {
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        CGPoint point = [gestureRecognizer locationInView:self.commentTableView];
...
- (void)deleteTopCommentAtIndex:(NSIndexPath*)indexPath {
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    NSString *cid = self.commentArray[indexPath.section].cid;
    NSString *urlString = [NSString stringWithFormat:@"https://travel.knoci.cn/comment/delete?cid=%@", cid];
    manager.requestSerializer = [AFJSONRequestSerializer serializer];//设置请求序列化器
    manager.responseSerializer = [AFJSONResponseSerializer serializer];//设置相应序列化器为 AFJSONResponseSerializer，用于将服务器返回的 JSON 数据转换为 NSDictionary 或 NSArray
    NSLog(@"%@", self.commentArray[indexPath.section].cid);
    [manager.requestSerializer setValue:self.authorization forHTTPHeaderField:@"Authorization"];
    [manager POST:urlString parameters:nil headers:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.commentArray removeObjectAtIndex:indexPath.section];
            [self.commentTableView beginUpdates];
            [self.commentTableView deleteSections:[NSIndexSet indexSetWithIndex:indexPath.section] withRowAnimation:UITableViewRowAnimationAutomatic];
            [self.commentTableView endUpdates];
        });
    } failure:^(NSURLSessionDataTask *task, NSError *error) {
        NSLog(@"%d",error.code);
...
- (void)deleteReplyAtIndex:(NSIndexPath*)indexPath {
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    NSString *cid = self.commentArray[indexPath.section].replies[indexPath.row - 1].cid;
    NSString *urlString = [NSString stringWithFormat:@"https://travel.knoci.cn/comment/delete?cid=%@", cid];
    manager.requestSerializer = [AFJSONRequestSerializer serializer];//设置请求序列化器
    manager.responseSerializer = [AFJSONResponseSerializer serializer];//设置相应序列化器为 AFJSONResponseSerializer，用于将服务器返回的 JSON 数据转换为 NSDictionary 或 NSArray
    [manager.requestSerializer setValue:self.authorization forHTTPHeaderField:@"Authorization"];
    [manager POST:urlString parameters:nil headers:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.commentArray[indexPath.section].replies removeObjectAtIndex:indexPath.row - 1];
            [self.commentTableView beginUpdates];
            [self.commentTableView reloadSections:[NSIndexSet indexSetWithIndex:indexPath.section] withRowAnimation:UITableViewRowAnimationAutomatic];
            [self.commentTableView endUpdates];
        });
    } failure:^(NSURLSessionDataTask *task, NSError *error) {
        NSLog(@"%d",error.code);
